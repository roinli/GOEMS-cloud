...
public class SecurityUtils
{
     ...
     * 是否为平台超级管理员。
     *
     * 除兼容历史上的固定用户ID外，还应以登录态中的超级管理员角色或通配权限为准。
     */
    public static boolean isSuperAdmin()
    {
        ...
        String contextPermission = SecurityContextHolder.getPermission();
        if (StringUtils.isNotEmpty(contextPermission))
        {
            for (String permission : contextPermission.split(","))
            {
        ...
        if (loginUser == null)
        {
            return isPlatformRootAccount(getUsername(), getTenantId());
        }
        ...
        Long tenantId = loginUser.getSysUser() == null ? null : loginUser.getSysUser().getTenantId();
        if (tenantId == null)
        {
            tenantId = getTenantId();
        }
        return isPlatformRootAccount(username, tenantId);
    }

    private static boolean isRootRole(String role)
    {
        ...
        return "admin".equalsIgnoreCase(key)
                || "root".equalsIgnoreCase(key)
                || "super_admin".equalsIgnoreCase(key)
                || "superadmin".equalsIgnoreCase(key)
                || "platform_admin".equalsIgnoreCase(key)
                || "platform-admin".equalsIgnoreCase(key);
    }

    private static boolean isPlatformRootAccount(String username, Long tenantId)
    {
        if (tenantId == null || tenantId.longValue() != 9999L || StringUtils.isEmpty(username))
        {
            return false;
        }
        return "admin".equalsIgnoreCase(username.trim()) || "root".equalsIgnoreCase(username.trim());
    }
