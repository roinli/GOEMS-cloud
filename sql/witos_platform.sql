...
INSERT INTO `sys_role` VALUES (130, '安装商管理员', 'ems_installer_admin', 130, '1', 1, 1, '0', '0', 0, 'admin', NOW(), '', NULL, '默认安装商管理员', 9999);
INSERT INTO `sys_role` VALUES (131, 'EMS公司管理员', 'ems_company_admin', 131, '1', 1, 1, '0', '0', 0, 'admin', NOW(), '', NULL, 'EMS公司管理员', 9999);
INSERT INTO `sys_role` VALUES (132, 'EMS运维人员', 'ems_operator', 132, '1', 1, 1, '0', '0', 0, 'admin', NOW(), '', NULL, 'EMS运维角色', 9999);
INSERT INTO `sys_role` VALUES (133, 'EMS业主', 'ems_owner', 133, '1', 1, 1, '0', '0', 0, 'admin', NOW(), '', NULL, 'EMS业主角色', 9999);
INSERT INTO `sys_role` VALUES (134, 'EMS浏览员', 'ems_viewer', 134, '1', 1, 1, '0', '0', 0, 'admin', NOW(), '', NULL, 'EMS只读角色', 9999);
COMMIT;
...
/*
 ...
 说明：
 1. 本脚本为设计稿 DDL；实现时需要合并到 ems-api/sql/witos_platform.sql 初始化脚本。
 2. 本版按 20260630(第一版)/PC 新原型调整。
 3. sys_tenant 表示安装商租户；EMS 业务表用 tenant_id 做安装商隔离。
 4. EMS 公司为 N 级公司树，不再绑定 sys_dept/root_dept_id。
 5. EMS 数据权限使用 ems_user_scope，不使用 sys_user.dept_id、sys_role.data_scope、@DataScope(dept_id)。
 6. 公司邮箱、共享 EMI、北向管理、审批任务、公司默认电价 Tab 一期不建表。
 7. EMS 不直接连接 Edge/InfluxDB/RRD4J，只保存 server 映射、快照、日志和聚合报表。
*/
...
-- ----------------------------
-- 1. 平台基础：公司树、用户扩展、授权范围、业务配置
-- ----------------------------

DROP TABLE IF EXISTS `ems_company`;
CREATE TABLE `ems_company` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '公司ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `parent_id` bigint NOT NULL DEFAULT 0 COMMENT '父公司ID，0表示根公司',
  `ancestors` varchar(512) DEFAULT '0' COMMENT '祖级列表，逗号分隔',
  `company_name` varchar(128) NOT NULL COMMENT '公司名称',
  `company_desc` varchar(512) DEFAULT NULL COMMENT '公司描述',
  ...
  `record_no` varchar(128) DEFAULT NULL COMMENT '备案号',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_ems_company_tenant_parent` (`tenant_id`, `parent_id`),
  KEY `idx_ems_company_tenant_name` (`tenant_id`, `company_name`),
  KEY `idx_ems_company_status` (`status`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS公司树表';

DROP TABLE IF EXISTS `ems_user_profile`;
CREATE TABLE `ems_user_profile` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `user_id` bigint NOT NULL COMMENT '系统用户ID',
  `primary_company_id` bigint NOT NULL DEFAULT 0 COMMENT '主归属公司ID；安装商管理员可为0',
  `is_default_installer_admin` char(1) DEFAULT '0' COMMENT '是否默认安装商管理员：1是，0否',
  `status` char(1) DEFAULT '0' COMMENT '状态：0正常，1停用',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_user_profile_user` (`tenant_id`, `user_id`),
  KEY `idx_ems_user_profile_company` (`tenant_id`, `primary_company_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS用户扩展表';

DROP TABLE IF EXISTS `ems_installer_admin_guard`;
CREATE TABLE `ems_installer_admin_guard` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `user_id` bigint NOT NULL COMMENT '默认安装商管理员用户ID',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_installer_admin_tenant` (`tenant_id`),
  UNIQUE KEY `uk_ems_installer_admin_user` (`tenant_id`, `user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS默认安装商管理员唯一约束表';

DROP TABLE IF EXISTS `ems_user_scope`;
CREATE TABLE `ems_user_scope` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `user_id` bigint NOT NULL COMMENT '系统用户ID',
  `scope_type` varchar(32) NOT NULL COMMENT '授权类型：COMPANY/STATION',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID，scope_type=COMPANY时必填',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID，scope_type=STATION时必填',
  `include_descendants` char(1) DEFAULT '0' COMMENT '公司授权是否包含后代：0包含，1不包含',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_user_scope` (`tenant_id`, `user_id`, `scope_type`, `company_id`, `station_id`),
  KEY `idx_ems_user_scope_company` (`tenant_id`, `company_id`),
  KEY `idx_ems_user_scope_station` (`tenant_id`, `station_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS用户数据授权范围表';

DROP TABLE IF EXISTS `ems_business_param`;
CREATE TABLE `ems_business_param` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID，0表示租户级',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID，0表示非电站级',
  `param_key` varchar(128) NOT NULL COMMENT '参数键',
  `param_value` varchar(1024) DEFAULT NULL COMMENT '参数值',
  `value_type` varchar(32) DEFAULT 'STRING' COMMENT '值类型：STRING/NUMBER/BOOLEAN/JSON',
  `scope_type` varchar(32) DEFAULT 'TENANT' COMMENT '作用域：TENANT/COMPANY/STATION',
  `status` char(1) DEFAULT '0' COMMENT '状态：0正常，1停用',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
...
CREATE TABLE `ems_metric_history_5min` (
  ...
  `source_role` varchar(64) DEFAULT NULL COMMENT '指标来源角色',
  `source_priority` int DEFAULT 100 COMMENT '同一业务指标多来源优先级，数值越小优先级越高',
  `bucket_time` datetime NOT NULL COMMENT '5分钟桶开始时间',
  `metric_value` decimal(24,8) DEFAULT NULL COMMENT '5分钟指标值，计算方法由sample_method定义',
  `metric_text` varchar(512) DEFAULT NULL COMMENT '文本值',
  `unit` varchar(32) DEFAULT NULL COMMENT '单位',
  `quality` varchar(32) DEFAULT 'GOOD' COMMENT '质量：GOOD/PARTIAL/BAD/MISSING',
  `quality_reason` varchar(512) DEFAULT NULL COMMENT '质量原因',
  `source_sample_time` datetime DEFAULT NULL COMMENT '源采样时间',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
...
CREATE TABLE `ems_sync_log` (
  ...
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_sync_log_scope` (`tenant_id`, `company_id`, `station_id`),
  KEY `idx_ems_sync_log_type_time` (`sync_type`, `started_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS同步日志表';

-- ----------------------------
-- 3. 电站、控制器、设备、电价
-- ----------------------------

DROP TABLE IF EXISTS `ems_station`;
CREATE TABLE `ems_station` (
  ...
  `station_type` varchar(32) DEFAULT 'STORAGE' COMMENT '电站类型：PV/STORAGE/PV_STORAGE/OTHER',
  `country` varchar(64) DEFAULT NULL COMMENT '国家/地区',
  `contact_name` varchar(64) DEFAULT NULL COMMENT '联系人',
  `contact_phone` varchar(32) DEFAULT NULL COMMENT '联系电话',
  `run_mode` varchar(32) DEFAULT 'GRID_CONNECTED' COMMENT '运行模式：GRID_CONNECTED/OFF_GRID/HYBRID',
  `timezone` varchar(64) DEFAULT NULL COMMENT '时区',
  `image_url` varchar(512) DEFAULT NULL COMMENT '站点图片',
  `capacity_kw` decimal(18,6) DEFAULT NULL COMMENT '装机容量kW',
  `address` varchar(512) DEFAULT NULL COMMENT '电站地址',
  `longitude` decimal(18,8) DEFAULT NULL COMMENT '经度',
  `latitude` decimal(18,8) DEFAULT NULL COMMENT '纬度',
  `commission_date` date DEFAULT NULL COMMENT '投运日期',
  `status` varchar(32) DEFAULT 'NORMAL' COMMENT '状态：NORMAL/DISABLED/CONSTRUCTION',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_station_code` (`tenant_id`, `station_code`),
  KEY `idx_ems_station_company` (`tenant_id`, `company_id`),
  KEY `idx_ems_station_status` (`status`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电站表';

DROP TABLE IF EXISTS `ems_device`;
CREATE TABLE `ems_device` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '设备ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `controller_id` bigint DEFAULT NULL COMMENT '所属控制器设备ID',
  `device_code` varchar(64) NOT NULL COMMENT '设备编码',
  `device_name` varchar(128) NOT NULL COMMENT '设备名称',
  `device_type` varchar(64) NOT NULL COMMENT '设备类型：CONTROLLER/INVERTER/PCS/ESS/METER/LOAD/OTHER',
  `rated_capacity` decimal(18,6) DEFAULT NULL COMMENT '额定容量',
  ...
  `firmware_version` varchar(128) DEFAULT NULL COMMENT '固件版本',
  `last_heartbeat_time` datetime DEFAULT NULL COMMENT '最后心跳时间',
  `controller_version` varchar(128) DEFAULT NULL COMMENT '控制器版本',
  `install_date` date DEFAULT NULL COMMENT '安装日期',
  `comm_status` varchar(32) DEFAULT 'UNKNOWN' COMMENT '通讯状态：ONLINE/OFFLINE/UNKNOWN',
  `status` varchar(32) DEFAULT 'NORMAL' COMMENT '设备状态：NORMAL/WARNING/FAULT/DISABLED',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_device_code` (`tenant_id`, `device_code`),
  KEY `idx_ems_device_station` (`tenant_id`, `station_id`),
  KEY `idx_ems_device_company` (`tenant_id`, `company_id`),
  KEY `idx_ems_device_type_status` (`device_type`, `status`),
  KEY `idx_ems_device_serial` (`serial_no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS设备表';

DROP TABLE IF EXISTS `ems_device_history`;
CREATE TABLE `ems_device_history` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_device_history_device_time` (`device_id`, `event_time`),
  KEY `idx_ems_device_history_scope` (`tenant_id`, `company_id`, `station_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS设备历史信息表';

DROP TABLE IF EXISTS `ems_price_rule`;
CREATE TABLE `ems_price_rule` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '电价规则ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '规则所属公司ID',
  `rule_name` varchar(128) NOT NULL COMMENT '规则名称',
  `price_type` varchar(32) NOT NULL COMMENT '电价类型：PURCHASE/FEED_IN',
  `price_mode` varchar(32) DEFAULT 'SIMPLE' COMMENT '电价模式：SIMPLE/TOU',
  `currency` varchar(16) DEFAULT 'CNY' COMMENT '币种',
  `base_price` decimal(18,6) DEFAULT NULL COMMENT '固定电价',
  `is_default` char(1) DEFAULT '1' COMMENT '是否默认规则：0是，1否',
  `source_rule_id` bigint DEFAULT NULL COMMENT '复制来源规则ID',
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  `active_default_guard` char(1) GENERATED ALWAYS AS (CASE WHEN `is_default` = '0' AND `del_flag` = '0' THEN '0' ELSE NULL END) STORED COMMENT '未删除默认规则唯一键',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_price_rule_active_default` (`tenant_id`, `company_id`, `price_type`, `active_default_guard`),
  KEY `idx_ems_price_rule_company` (`tenant_id`, `company_id`),
  KEY `idx_ems_price_rule_type_status` (`price_type`, `status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电价规则表';

DROP TABLE IF EXISTS `ems_price_period`;
CREATE TABLE `ems_price_period` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `rule_id` bigint NOT NULL COMMENT '电价规则ID',
  `period_type` varchar(32) DEFAULT 'NORMAL' COMMENT '时段类型：PEAK/FLAT/VALLEY/NORMAL',
  `start_time` time NOT NULL COMMENT '开始时间',
  `end_time` time NOT NULL COMMENT '结束时间',
  `price` decimal(18,6) NOT NULL COMMENT '电价',
  `weekday_mask` varchar(32) DEFAULT '1111111' COMMENT '星期掩码，周一到周日',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_price_period_rule` (`tenant_id`, `rule_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS分时电价时段表';

DROP TABLE IF EXISTS `ems_price_apply`;
CREATE TABLE `ems_price_apply` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '电价应用ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '所属公司ID',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID，0表示公司级应用',
  `rule_id` bigint NOT NULL COMMENT '电价规则ID',
  `price_type` varchar(32) NOT NULL COMMENT '电价类型：PURCHASE/FEED_IN',
  `effective_start` datetime NOT NULL COMMENT '生效开始时间',
  `effective_end` datetime DEFAULT NULL COMMENT '生效结束时间',
  `permanent` char(1) DEFAULT '0' COMMENT '是否永久：0是，1否',
  `status` varchar(32) DEFAULT 'ENABLED' COMMENT '状态：ENABLED/DISABLED/EXPIRED',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_price_apply_station` (`tenant_id`, `station_id`, `price_type`),
  KEY `idx_ems_price_apply_company` (`tenant_id`, `company_id`, `price_type`),
  KEY `idx_ems_price_apply_effective` (`effective_start`, `effective_end`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电价应用设置表';

-- ----------------------------
-- 4. 监控和快照
-- ----------------------------

DROP TABLE IF EXISTS `ems_dashboard_config`;
CREATE TABLE `ems_dashboard_config` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `user_id` bigint NOT NULL DEFAULT 0 COMMENT '用户ID，0表示租户默认配置',
  `dashboard_type` varchar(64) NOT NULL COMMENT '看板类型：HOME/LIST/KPI/MAP/BIG_SCREEN/MONITOR',
  `config_json` text COMMENT '看板配置JSON',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_dashboard_config_scope` (`tenant_id`, `user_id`, `dashboard_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS看板配置表';

DROP TABLE IF EXISTS `ems_metric_snapshot`;
CREATE TABLE `ems_metric_snapshot` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `device_id` bigint NOT NULL DEFAULT 0 COMMENT '设备ID，0表示电站级指标',
  `metric_key` varchar(128) NOT NULL COMMENT '指标编码',
  `metric_value` decimal(24,8) DEFAULT NULL COMMENT '数值型指标值',
  `metric_text` varchar(256) DEFAULT NULL COMMENT '文本型指标值',
  `unit` varchar(32) DEFAULT NULL COMMENT '单位',
  `sample_time` datetime NOT NULL COMMENT '采样时间',
  `quality` varchar(32) DEFAULT 'GOOD' COMMENT '数据质量：GOOD/STALE/BAD/MISSING',
  `quality_reason` varchar(256) DEFAULT NULL COMMENT '质量原因说明',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_metric_snapshot_metric` (`tenant_id`, `station_id`, `device_id`, `metric_key`),
  KEY `idx_ems_metric_snapshot_scope` (`tenant_id`, `company_id`, `station_id`),
  KEY `idx_ems_metric_snapshot_sample` (`sample_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS实时指标快照表';

DROP TABLE IF EXISTS `ems_energy_flow_session`;
CREATE TABLE `ems_energy_flow_session` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `user_id` bigint DEFAULT NULL COMMENT '开启用户ID',
  `session_seconds` int NOT NULL DEFAULT 60 COMMENT '会话时长秒',
  `started_at` datetime NOT NULL COMMENT '开启时间',
  `expires_at` datetime NOT NULL COMMENT '过期时间',
  `session_status` varchar(32) DEFAULT 'ACTIVE' COMMENT '状态：ACTIVE/EXPIRED/CLOSED',
  `request_source` varchar(32) DEFAULT 'PC' COMMENT '来源：PC/API',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_energy_flow_session_station_time` (`tenant_id`, `station_id`, `started_at`),
  KEY `idx_ems_energy_flow_session_user_time` (`tenant_id`, `user_id`, `started_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS实时能量流会话记录表';

-- ----------------------------
-- 5. 报表
-- ----------------------------

DROP TABLE IF EXISTS `ems_report_station_hour`;
CREATE TABLE `ems_report_station_hour` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `stat_time` datetime NOT NULL COMMENT '统计小时',
  `generation_kwh` decimal(24,8) DEFAULT NULL COMMENT '发电量kWh',
  `charge_kwh` decimal(24,8) DEFAULT NULL COMMENT '储能充电量kWh',
  `discharge_kwh` decimal(24,8) DEFAULT NULL COMMENT '储能放电量kWh',
  `consumption_kwh` decimal(24,8) DEFAULT NULL COMMENT '用电量kWh',
  `grid_import_kwh` decimal(24,8) DEFAULT NULL COMMENT '购电量kWh',
  `grid_export_kwh` decimal(24,8) DEFAULT NULL COMMENT '上网电量kWh',
  `revenue_amount` decimal(24,8) DEFAULT NULL COMMENT '收益金额',
  `feed_in_revenue` decimal(24,8) DEFAULT NULL COMMENT '上网收益金额',
  `self_use_saving` decimal(24,8) DEFAULT NULL COMMENT '自发自用节省金额',
  `storage_arbitrage_revenue` decimal(24,8) DEFAULT NULL COMMENT '储能套利收益金额',
  `purchase_cost` decimal(24,8) DEFAULT NULL COMMENT '购电成本金额，单列展示不从收益扣除',
  `revenue_quality_reason` varchar(512) DEFAULT NULL COMMENT '收益计算口径与质量说明',
  `equivalent_hours` decimal(18,6) DEFAULT NULL COMMENT '等效利用小时',
  `co2_reduction_kg` decimal(24,8) DEFAULT NULL COMMENT '碳减排kg',
  `standard_coal_kg` decimal(24,8) DEFAULT NULL COMMENT '节约标准煤kg',
  `equivalent_trees` decimal(24,8) DEFAULT NULL COMMENT '等效植树棵数',
  `data_quality` varchar(32) DEFAULT 'GOOD' COMMENT '数据质量：GOOD/PARTIAL/MISSING/BAD',
  `quality_reason` varchar(256) DEFAULT NULL COMMENT '质量原因说明',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_report_station_hour` (`tenant_id`, `station_id`, `stat_time`),
  KEY `idx_ems_report_station_hour_scope` (`tenant_id`, `company_id`, `stat_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电站小时报表';

DROP TABLE IF EXISTS `ems_report_station_day`;
CREATE TABLE `ems_report_station_day` LIKE `ems_report_station_hour`;
ALTER TABLE `ems_report_station_day` COMMENT='EMS电站日报表';

DROP TABLE IF EXISTS `ems_report_station_month`;
CREATE TABLE `ems_report_station_month` LIKE `ems_report_station_hour`;
ALTER TABLE `ems_report_station_month` COMMENT='EMS电站月报表';

DROP TABLE IF EXISTS `ems_report_station_year`;
CREATE TABLE `ems_report_station_year` LIKE `ems_report_station_hour`;
ALTER TABLE `ems_report_station_year` COMMENT='EMS电站年报表';

DROP TABLE IF EXISTS `ems_report_device_hour`;
CREATE TABLE `ems_report_device_hour` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  ...
  `stat_time` datetime NOT NULL COMMENT '统计小时',
  `generation_kwh` decimal(24,8) DEFAULT NULL COMMENT '发电量kWh',
  `charge_kwh` decimal(24,8) DEFAULT NULL COMMENT '充电量kWh',
  `discharge_kwh` decimal(24,8) DEFAULT NULL COMMENT '放电量kWh',
  `runtime_hours` decimal(18,6) DEFAULT NULL COMMENT '运行小时',
  `peak_power_kw` decimal(18,6) DEFAULT NULL COMMENT '峰值功率kW',
  `avg_soc` decimal(18,6) DEFAULT NULL COMMENT '平均SOC',
  `avg_soh` decimal(18,6) DEFAULT NULL COMMENT '平均SOH',
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_report_device_hour` (`tenant_id`, `device_id`, `stat_time`),
  KEY `idx_ems_report_device_hour_station_time` (`tenant_id`, `station_id`, `stat_time`),
  KEY `idx_ems_report_device_hour_company_time` (`tenant_id`, `company_id`, `stat_time`),
  KEY `idx_ems_report_device_hour_type` (`device_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS设备小时报表';

DROP TABLE IF EXISTS `ems_report_device_day`;
CREATE TABLE `ems_report_device_day` LIKE `ems_report_device_hour`;
ALTER TABLE `ems_report_device_day` COMMENT='EMS设备日报表';

DROP TABLE IF EXISTS `ems_report_device_month`;
CREATE TABLE `ems_report_device_month` LIKE `ems_report_device_hour`;
ALTER TABLE `ems_report_device_month` COMMENT='EMS设备月报表';

DROP TABLE IF EXISTS `ems_report_device_year`;
CREATE TABLE `ems_report_device_year` LIKE `ems_report_device_hour`;
ALTER TABLE `ems_report_device_year` COMMENT='EMS设备年报表';

DROP TABLE IF EXISTS `ems_report_sync_task`;
CREATE TABLE `ems_report_sync_task` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID',
  `report_type` varchar(32) NOT NULL COMMENT '报表类型',
  `period_type` varchar(16) NOT NULL COMMENT '聚合粒度',
  `range_start_time` datetime NOT NULL COMMENT '补拉开始时间',
  `range_end_time` datetime NOT NULL COMMENT '补拉结束时间',
  `task_key` varchar(256) NOT NULL COMMENT '幂等任务键',
  `task_status` varchar(32) NOT NULL DEFAULT 'PENDING' COMMENT '任务状态',
  `retry_count` int NOT NULL DEFAULT 0 COMMENT '重试次数',
  `affected_rows` int NOT NULL DEFAULT 0 COMMENT '影响记录数',
  `source_system` varchar(32) NOT NULL DEFAULT 'SERVER' COMMENT '来源系统',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ems_report_sync_task_key` (`tenant_id`, `task_key`),
  KEY `idx_ems_report_sync_task_scope` (`tenant_id`, `company_id`, `station_id`),
  KEY `idx_ems_report_sync_task_status` (`task_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS报表历史补拉任务表';

-- ----------------------------
-- 6. 告警
-- ----------------------------

DROP TABLE IF EXISTS `ems_alarm_rule`;
CREATE TABLE `ems_alarm_rule` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '告警规则ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID，0表示租户级规则',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID，0表示非电站级规则',
  `rule_name` varchar(128) NOT NULL COMMENT '规则名称',
  `metric_key` varchar(128) NOT NULL COMMENT '指标编码',
  `device_type` varchar(64) DEFAULT NULL COMMENT '适用设备类型',
  `condition_op` varchar(16) NOT NULL COMMENT '条件操作符：GT/GE/LT/LE/EQ/NE',
  `threshold_value` decimal(24,8) DEFAULT NULL COMMENT '阈值',
  `duration_seconds` int DEFAULT 0 COMMENT '持续时间秒',
  `severity` varchar(32) DEFAULT 'WARNING' COMMENT '告警等级：INFO/WARNING/FAULT/CRITICAL',
  `enabled` char(1) DEFAULT '0' COMMENT '是否启用：0启用，1停用',
  `notify_enabled` char(1) DEFAULT '1' COMMENT '是否通知：0通知，1不通知',
  `notify_channels` varchar(255) DEFAULT 'EMAIL' COMMENT '通知通道，逗号分隔',
  `notify_targets` varchar(1024) DEFAULT NULL COMMENT '通知对象，逗号分隔',
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在，2删除',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_alarm_rule_metric` (`metric_key`),
  KEY `idx_ems_alarm_rule_scope` (`tenant_id`, `company_id`, `station_id`, `device_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS告警规则表';

DROP TABLE IF EXISTS `ems_alarm_event`;
CREATE TABLE `ems_alarm_event` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '告警事件ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `device_id` bigint DEFAULT NULL COMMENT '设备ID',
  `rule_id` bigint DEFAULT NULL COMMENT '告警规则ID',
  `alarm_code` varchar(128) NOT NULL COMMENT '告警编码',
  `alarm_name` varchar(128) DEFAULT NULL COMMENT '告警名称',
  `severity` varchar(32) DEFAULT 'WARNING' COMMENT '告警等级',
  `alarm_status` varchar(32) DEFAULT 'ACTIVE' COMMENT '告警状态：ACTIVE/ACKED/CLEARED',
  `occurrence_count` int DEFAULT 1 COMMENT '重复告警累计次数',
  `first_time` datetime NOT NULL COMMENT '首次发生时间',
  `last_time` datetime DEFAULT NULL COMMENT '最后发生时间',
  `clear_time` datetime DEFAULT NULL COMMENT '清除时间',
  `clear_type` varchar(32) DEFAULT NULL COMMENT '清除类型：AUTO/MANUAL',
  `clear_by` varchar(64) DEFAULT NULL COMMENT '清除人',
  `ack_time` datetime DEFAULT NULL COMMENT '确认时间',
  `ack_by` varchar(64) DEFAULT NULL COMMENT '确认人',
  `source_type` varchar(32) DEFAULT 'EMS' COMMENT '来源类型：EMS/SERVER',
  `source_payload` text COMMENT '来源原始数据JSON',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_ems_alarm_event_scope_time` (`tenant_id`, `company_id`, `station_id`, `first_time`),
  KEY `idx_ems_alarm_event_device` (`device_id`),
  KEY `idx_ems_alarm_event_status` (`alarm_status`, `severity`),
  KEY `idx_ems_alarm_event_code` (`alarm_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS告警事件表';

DROP TABLE IF EXISTS `ems_alarm_notify_config`;
CREATE TABLE `ems_alarm_notify_config` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID，0表示租户级',
  `severity` varchar(32) NOT NULL COMMENT '告警等级',
  `channel_type` varchar(32) NOT NULL COMMENT '通知渠道：MAIL/SMS/WEBHOOK',
  `receiver_config` text COMMENT '接收人配置JSON',
  `enabled` char(1) DEFAULT '0' COMMENT '是否启用：0启用，1停用',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_alarm_notify_scope` (`tenant_id`, `company_id`, `severity`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS告警通知配置表';

DROP TABLE IF EXISTS `ems_alarm_mail_config`;
CREATE TABLE `ems_alarm_mail_config` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID，0表示租户级',
  ...
  `smtp_password` varchar(255) DEFAULT NULL COMMENT 'SMTP密码或授权码',
  `from_address` varchar(255) DEFAULT NULL COMMENT '发件邮箱',
  `from_name` varchar(128) DEFAULT NULL COMMENT '发件人名称',
  `ssl_enabled` char(1) DEFAULT '0' COMMENT 'SSL开关：1开0关',
  `starttls_enabled` char(1) DEFAULT '0' COMMENT 'STARTTLS开关：1开0关',
  `auth_enabled` char(1) DEFAULT '1' COMMENT 'SMTP认证开关：1开0关',
  `enabled` char(1) DEFAULT '1' COMMENT '是否启用：1启用0停用',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_alarm_mail_config_scope` (`tenant_id`, `company_id`, `enabled`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS告警邮件配置表';

DROP TABLE IF EXISTS `ems_alarm_notify_log`;
CREATE TABLE `ems_alarm_notify_log` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '安装商租户ID',
  `company_id` bigint NOT NULL DEFAULT 0 COMMENT '公司ID',
  `station_id` bigint NOT NULL DEFAULT 0 COMMENT '电站ID',
  ...
  `channel_type` varchar(32) NOT NULL COMMENT '通知渠道：EMAIL/SMS/INAPP/WECOM',
  `receiver` varchar(255) DEFAULT NULL COMMENT '接收人',
  `send_status` varchar(32) NOT NULL COMMENT '发送状态：SUCCESS/FAILED/SKIPPED',
  ...
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  KEY `idx_ems_alarm_notify_log_event` (`tenant_id`, `event_id`, `channel_type`),
  KEY `idx_ems_alarm_notify_log_time` (`tenant_id`, `triggered_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS告警通知日志表';

DROP TABLE IF EXISTS `ems_station_view_tab`;
CREATE TABLE `ems_station_view_tab` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `company_id` bigint NOT NULL COMMENT '公司ID',
  `station_id` bigint NOT NULL COMMENT '电站ID',
  `tab_name` varchar(128) NOT NULL COMMENT '页签名称',
  `view_type` varchar(32) NOT NULL COMMENT '视图类型：PHYSICAL/LOGIC',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_ems_station_view_tab_scope` (`tenant_id`, `station_id`, `view_type`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电站视图页签表';

DROP TABLE IF EXISTS `ems_station_view_node`;
CREATE TABLE `ems_station_view_node` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `tab_id` bigint NOT NULL COMMENT '页签ID',
  `node_type` varchar(64) NOT NULL COMMENT '节点类型',
  `device_id` bigint DEFAULT NULL COMMENT '设备ID',
  `component_id` bigint DEFAULT NULL COMMENT '组件ID',
  `x` decimal(18,6) DEFAULT NULL COMMENT 'X坐标',
  `y` decimal(18,6) DEFAULT NULL COMMENT 'Y坐标',
  `width` decimal(18,6) DEFAULT NULL COMMENT '宽度',
  `height` decimal(18,6) DEFAULT NULL COMMENT '高度',
  `node_style_json` text COMMENT '节点样式JSON',
  `sort_no` int DEFAULT 1 COMMENT '排序',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在2删除',
  PRIMARY KEY (`id`),
  KEY `idx_ems_station_view_node_tab` (`tenant_id`, `tab_id`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电站视图节点表';

DROP TABLE IF EXISTS `ems_station_view_relation`;
CREATE TABLE `ems_station_view_relation` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `tab_id` bigint NOT NULL COMMENT '页签ID',
  `source_node_id` bigint NOT NULL COMMENT '源节点ID',
  `target_node_id` bigint NOT NULL COMMENT '目标节点ID',
  `relation_type` varchar(64) DEFAULT 'CONNECT' COMMENT '关系类型',
  `line_style_json` text COMMENT '连线样式JSON',
  `sort_no` int DEFAULT 1 COMMENT '排序',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  ...
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在2删除',
  PRIMARY KEY (`id`),
  KEY `idx_ems_station_view_relation_tab` (`tenant_id`, `tab_id`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS电站视图关系表';

DROP TABLE IF EXISTS `ems_optimizer_component_binding`;
CREATE TABLE `ems_optimizer_component_binding` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `tab_id` bigint NOT NULL COMMENT '页签ID',
  `optimizer_device_id` bigint NOT NULL COMMENT '优化器设备ID',
  `module_device_id` bigint DEFAULT NULL COMMENT '组件设备ID',
  `component_id` bigint DEFAULT NULL COMMENT '组件ID',
  `binding_order` int DEFAULT 1 COMMENT '绑定顺序',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  ...
  `del_flag` char(1) DEFAULT '0' COMMENT '删除标志：0存在2删除',
  PRIMARY KEY (`id`),
  KEY `idx_ems_optimizer_component_binding_tab` (`tenant_id`, `tab_id`, `del_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='EMS优化器组件绑定表';
...
(4000, '数据总览', 0, 10, 'ems-home', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'dashboard', 'admin', NOW(), '', NULL, 'EMS数据总览目录'),
(4001, '列表视图', 4000, 1, 'list', 'ems/home/list', NULL, 1, 0, 'C', '0', '0', 'ems:home:list', 'list', 'admin', NOW(), '', NULL, ''),
(4100, '监控管理', 0, 20, 'ems-monitor', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'monitor', 'admin', NOW(), '', NULL, 'EMS监控管理目录'),
(4101, '监控概览', 4100, 1, 'overview', 'ems/monitor/overview', NULL, 1, 0, 'C', '0', '0', 'ems:monitor:overview', 'monitor', 'admin', NOW(), '', NULL, ''),
(4200, '电站管理', 0, 30, 'ems-station', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'tree', 'admin', NOW(), '', NULL, 'EMS电站管理目录'),
(4201, '电站管理', 4200, 1, 'index', 'ems/station/index', NULL, 1, 0, 'C', '0', '0', 'ems:station:list', 'tree', 'admin', NOW(), '', NULL, ''),
(4202, '新增电站', 4200, 2, 'create', 'ems/station/create', NULL, 1, 0, 'C', '1', '0', 'ems:station:add', 'edit', 'admin', NOW(), '', NULL, '隐藏创建页'),
(4207, '详细地址', 4200, 7, 'address', 'ems/station/address', NULL, 1, 0, 'C', '1', '0', 'ems:station:add', 'map-pin', 'admin', NOW(), '', NULL, '隐藏详细地址页'),
(4300, '设备管理', 0, 40, 'ems-device', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'server', 'admin', NOW(), '', NULL, 'EMS设备管理目录'),
(4301, '设备管理', 4300, 1, 'index', 'ems/device/index', NULL, 1, 0, 'C', '0', '0', 'ems:device:list', 'server', 'admin', NOW(), '', NULL, '开源版设备模拟管理'),
(4400, '运维管理', 0, 50, 'ems-ops', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'tool', 'admin', NOW(), '', NULL, 'EMS运维管理目录'),
(4405, '告警管理', 4400, 5, 'alarm-current', 'ems/ops/alarm-current', NULL, 1, 0, 'C', '0', '0', 'ems:alarm:current', 'bell-ring', 'admin', NOW(), '', NULL, ''),
(4500, '系统管理', 0, 60, 'ems-system', 'Layout', NULL, 1, 0, 'M', '0', '0', '', 'system', 'admin', NOW(), '', NULL, 'EMS系统管理目录'),
(4501, '公司管理', 4500, 1, 'company', 'ems/system/company', NULL, 1, 0, 'C', '0', '0', 'ems:company:list', 'tree', 'admin', NOW(), '', NULL, ''),
(4502, '公司编辑', 4500, 2, 'company-edit', 'ems/system/company-edit', NULL, 1, 0, 'C', '1', '0', 'ems:company:edit', 'edit', 'admin', NOW(), '', NULL, '隐藏编辑页'),
(4503, '公司信息', 4500, 3, 'company-info', 'ems/system/company-info', NULL, 1, 0, 'C', '1', '0', 'ems:company:info', 'form', 'admin', NOW(), '', NULL, '隐藏信息页'),
(4504, '员工管理', 4500, 4, 'employee', 'ems/system/employee', NULL, 1, 0, 'C', '0', '0', 'ems:employee:list', 'user', 'admin', NOW(), '', NULL, ''),
(4505, '业务配置', 4500, 5, 'business-config', 'ems/system/business-config', NULL, 1, 0, 'C', '0', '0', 'ems:business-config:list', 'file-text', 'admin', NOW(), '', NULL, ''),
(4506, '新增公司', 4500, 6, 'company-add', 'ems/system/company-edit', NULL, 1, 0, 'C', '1', '0', 'ems:company:add', 'plus-square', 'admin', NOW(), '', NULL, '隐藏新增公司页'),
(4507, '新增员工', 4500, 7, 'employee-add', 'ems/system/employee-edit', NULL, 1, 0, 'C', '1', '0', 'ems:employee:add', 'plus-square', 'admin', NOW(), '', NULL, '隐藏新增员工页'),
(4508, '修改员工', 4500, 8, 'employee-edit', 'ems/system/employee-edit', NULL, 1, 0, 'C', '1', '0', 'ems:employee:edit', 'edit', 'admin', NOW(), '', NULL, '隐藏修改员工页'),
(4509, '重置员工密码', 4500, 9, 'employee-password', 'ems/system/employee-password', NULL, 1, 0, 'C', '1', '0', 'ems:employee:resetPwd', 'key', 'admin', NOW(), '', NULL, '隐藏重置密码页'),
(4510, '新增业务配置', 4500, 10, 'business-config-add', 'ems/system/business-config-edit', NULL, 1, 0, 'C', '1', '0', 'ems:business-config:add', 'plus-square', 'admin', NOW(), '', NULL, '隐藏新增业务配置页');

INSERT INTO `sys_menu` VALUES
(5001, '公司新增', 4501, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:company:add', '#', 'admin', NOW(), '', NULL, ''),
(5002, '公司修改', 4501, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:company:edit', '#', 'admin', NOW(), '', NULL, ''),
(5003, '公司删除', 4501, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:company:remove', '#', 'admin', NOW(), '', NULL, ''),
(5004, '公司导出', 4501, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:company:export', '#', 'admin', NOW(), '', NULL, ''),
(5011, '员工新增', 4504, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:employee:add', '#', 'admin', NOW(), '', NULL, ''),
(5012, '员工修改', 4504, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:employee:edit', '#', 'admin', NOW(), '', NULL, ''),
(5013, '员工删除', 4504, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:employee:remove', '#', 'admin', NOW(), '', NULL, ''),
(5014, '员工重置密码', 4504, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:employee:resetPwd', '#', 'admin', NOW(), '', NULL, ''),
(5021, '电站新增', 4201, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:station:add', '#', 'admin', NOW(), '', NULL, ''),
(5022, '电站修改', 4201, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:station:edit', '#', 'admin', NOW(), '', NULL, ''),
(5023, '电站删除', 4201, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:station:remove', '#', 'admin', NOW(), '', NULL, ''),
(5024, '电站导出', 4201, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:station:export', '#', 'admin', NOW(), '', NULL, ''),
(5071, '设备新增', 4301, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:device:add', '#', 'admin', NOW(), '', NULL, ''),
(5072, '设备修改', 4301, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:device:edit', '#', 'admin', NOW(), '', NULL, ''),
(5073, '设备删除', 4301, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:device:remove', '#', 'admin', NOW(), '', NULL, ''),
(5051, '告警新增', 4405, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:alarm:add', '#', 'admin', NOW(), '', NULL, ''),
(5052, '告警修改', 4405, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:alarm:edit', '#', 'admin', NOW(), '', NULL, ''),
(5053, '告警删除', 4405, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:alarm:remove', '#', 'admin', NOW(), '', NULL, ''),
(5054, '告警确认', 4405, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:alarm:ack', '#', 'admin', NOW(), '', NULL, ''),
(5055, '告警清除', 4405, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:alarm:clear', '#', 'admin', NOW(), '', NULL, ''),
(5061, '业务配置新增', 4505, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:business-config:add', '#', 'admin', NOW(), '', NULL, ''),
(5062, '业务配置修改', 4505, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:business-config:edit', '#', 'admin', NOW(), '', NULL, ''),
(5063, '业务配置删除', 4505, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:business-config:remove', '#', 'admin', NOW(), '', NULL, ''),
(5064, '业务配置导出', 4505, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'ems:business-config:export', '#', 'admin', NOW(), '', NULL, '');
COMMIT;

INSERT INTO `sys_tenant_package` VALUES (103, 'EMS基础套餐', '4000,4001,4100,4101,4200,4201,4202,4207,4300,4301,4400,4405,4500,4501,4502,4503,4504,4505,4506,4507,4508,4509,4510,5001,5002,5003,5004,5011,5012,5013,5014,5021,5022,5023,5024,5051,5052,5053,5054,5055,5061,5062,5063,5064,5071,5072,5073', '0', '0', 'admin', NOW(), '', NULL, 'EMS初始化套餐');

UPDATE `sys_tenant_package`
SET `menu_ids` = CONCAT_WS(',', `menu_ids`, '4207,4405,4506,4507,4508,4509,4510,5051,5052,5053,5054,5055')
WHERE `id` = 103
  AND FIND_IN_SET('4207', `menu_ids`) = 0;

INSERT INTO `sys_tenant_package`
(`id`, `name`, `menu_ids`, `status`, `del_flag`, `create_by`, `create_time`, `update_by`, `update_time`, `remark`)
SELECT 104,
       '注册安装商套餐',
       GROUP_CONCAT(`menu_id` ORDER BY `menu_id` SEPARATOR ','),
       '0',
       '0',
       'admin',
       NOW(),
       'admin',
       NOW(),
       '注册安装商默认套餐；仅不授予KPI视图、地图模式、数据大屏；无支付能力'
FROM `sys_menu`
WHERE `menu_id` BETWEEN 4000 AND 5999
  AND (`perms` IS NULL OR `perms` NOT IN ('ems:home:kpi', 'ems:home:map', 'ems:home:dashboard'))
...
(2, 4000, 9999), (2, 4001, 9999), (2, 4002, 9999), (2, 4003, 9999), (2, 4004, 9999),
(2, 4100, 9999), (2, 4101, 9999), (2, 4102, 9999), (2, 4103, 9999), (2, 4104, 9999), (2, 4105, 9999), (2, 4106, 9999),
(2, 4200, 9999), (2, 4201, 9999), (2, 4202, 9999), (2, 4203, 9999), (2, 4204, 9999), (2, 4205, 9999),
(2, 4300, 9999), (2, 4301, 9999), (2, 4302, 9999), (2, 4303, 9999), (2, 4304, 9999),
(2, 4400, 9999), (2, 4401, 9999), (2, 4402, 9999), (2, 4403, 9999), (2, 4404, 9999),
(2, 4500, 9999), (2, 4501, 9999), (2, 4502, 9999), (2, 4503, 9999), (2, 4504, 9999), (2, 4505, 9999),
(2, 5001, 9999), (2, 5002, 9999), (2, 5003, 9999), (2, 5004, 9999),
(2, 5011, 9999), (2, 5012, 9999), (2, 5013, 9999), (2, 5014, 9999),
(2, 5021, 9999), (2, 5022, 9999), (2, 5023, 9999), (2, 5024, 9999),
(2, 5031, 9999), (2, 5032, 9999), (2, 5033, 9999), (2, 5034, 9999),
(2, 5041, 9999), (2, 5042, 9999), (2, 5043, 9999), (2, 5044, 9999), (2, 5045, 9999),
(2, 5071, 9999), (2, 5072, 9999), (2, 5073, 9999), (2, 5074, 9999), (2, 5075, 9999),
(2, 5051, 9999), (2, 5052, 9999), (2, 5053, 9999), (2, 5054, 9999), (2, 5055, 9999), (2, 5056, 9999), (2, 5057, 9999), (2, 5058, 9999), (2, 5059, 9999),
(2, 5061, 9999), (2, 5062, 9999), (2, 5063, 9999), (2, 5064, 9999);

DELETE FROM `sys_role_menu` WHERE `role_id` IN (130, 131, 132, 133, 134) AND `tenant_id` = 9999;

INSERT INTO `sys_role_menu` VALUES
(130, 4000, 9999), (130, 4001, 9999), (130, 4002, 9999), (130, 4003, 9999), (130, 4004, 9999),
(130, 4100, 9999), (130, 4101, 9999), (130, 4102, 9999), (130, 4103, 9999), (130, 4104, 9999), (130, 4105, 9999), (130, 4106, 9999),
(130, 4200, 9999), (130, 4201, 9999), (130, 4202, 9999), (130, 4203, 9999), (130, 4204, 9999), (130, 4205, 9999),
(130, 4300, 9999), (130, 4301, 9999), (130, 4302, 9999), (130, 4303, 9999), (130, 4304, 9999),
(130, 4400, 9999), (130, 4401, 9999), (130, 4402, 9999), (130, 4403, 9999), (130, 4404, 9999),
(130, 4500, 9999), (130, 4501, 9999), (130, 4502, 9999), (130, 4503, 9999), (130, 4504, 9999), (130, 4505, 9999),
(130, 5001, 9999), (130, 5002, 9999), (130, 5003, 9999), (130, 5004, 9999),
(130, 5011, 9999), (130, 5012, 9999), (130, 5013, 9999), (130, 5014, 9999),
(130, 5021, 9999), (130, 5022, 9999), (130, 5023, 9999), (130, 5024, 9999),
(130, 5031, 9999), (130, 5032, 9999), (130, 5033, 9999), (130, 5034, 9999),
(130, 5041, 9999), (130, 5042, 9999), (130, 5043, 9999), (130, 5044, 9999), (130, 5045, 9999),
(130, 5071, 9999), (130, 5072, 9999), (130, 5073, 9999), (130, 5074, 9999), (130, 5075, 9999),
(130, 5051, 9999), (130, 5052, 9999), (130, 5053, 9999), (130, 5054, 9999), (130, 5055, 9999), (130, 5056, 9999), (130, 5057, 9999), (130, 5058, 9999), (130, 5059, 9999),
(130, 5061, 9999), (130, 5062, 9999), (130, 5063, 9999), (130, 5064, 9999),
(130, 4520, 9999), (130, 4521, 9999), (130, 5081, 9999), (130, 5082, 9999), (130, 5083, 9999), (130, 5084, 9999), (130, 5085, 9999),
(131, 4000, 9999), (131, 4001, 9999), (131, 4002, 9999), (131, 4003, 9999), (131, 4004, 9999),
(131, 4100, 9999), (131, 4101, 9999), (131, 4102, 9999), (131, 4103, 9999), (131, 4104, 9999), (131, 4105, 9999), (131, 4106, 9999),
(131, 4200, 9999), (131, 4201, 9999), (131, 4202, 9999), (131, 4203, 9999), (131, 4204, 9999), (131, 4205, 9999), (131, 4206, 9999),
(131, 4300, 9999), (131, 4301, 9999), (131, 4302, 9999), (131, 4303, 9999), (131, 4304, 9999),
(131, 4400, 9999), (131, 4401, 9999), (131, 4402, 9999), (131, 4403, 9999), (131, 4404, 9999),
(131, 4500, 9999), (131, 4501, 9999), (131, 4502, 9999), (131, 4503, 9999), (131, 4504, 9999),
(131, 5001, 9999), (131, 5002, 9999), (131, 5003, 9999), (131, 5004, 9999),
(131, 5011, 9999), (131, 5012, 9999), (131, 5013, 9999), (131, 5014, 9999),
(131, 5021, 9999), (131, 5022, 9999), (131, 5023, 9999), (131, 5024, 9999),
(131, 5031, 9999), (131, 5032, 9999), (131, 5033, 9999), (131, 5034, 9999),
(131, 5041, 9999), (131, 5042, 9999), (131, 5043, 9999), (131, 5044, 9999), (131, 5045, 9999),
(131, 5071, 9999), (131, 5072, 9999), (131, 5073, 9999), (131, 5074, 9999), (131, 5075, 9999),
(131, 5051, 9999), (131, 5052, 9999), (131, 5053, 9999), (131, 5054, 9999), (131, 5055, 9999), (131, 5056, 9999), (131, 5057, 9999), (131, 5058, 9999), (131, 5059, 9999),
(132, 4000, 9999), (132, 4001, 9999), (132, 4002, 9999), (132, 4003, 9999), (132, 4004, 9999),
(132, 4100, 9999), (132, 4101, 9999), (132, 4102, 9999), (132, 4103, 9999), (132, 4104, 9999), (132, 4105, 9999), (132, 4106, 9999),
(132, 4200, 9999), (132, 4201, 9999), (132, 4202, 9999), (132, 4203, 9999), (132, 4204, 9999), (132, 4205, 9999), (132, 4206, 9999),
(132, 4300, 9999), (132, 4301, 9999), (132, 4302, 9999), (132, 4303, 9999), (132, 4304, 9999),
(132, 4400, 9999), (132, 4401, 9999), (132, 4402, 9999), (132, 4403, 9999), (132, 4404, 9999),
(132, 5021, 9999), (132, 5022, 9999), (132, 5023, 9999), (132, 5024, 9999),
(132, 5031, 9999), (132, 5032, 9999), (132, 5033, 9999), (132, 5034, 9999),
(132, 5041, 9999), (132, 5042, 9999), (132, 5043, 9999), (132, 5044, 9999), (132, 5045, 9999),
(132, 5071, 9999), (132, 5072, 9999), (132, 5073, 9999), (132, 5074, 9999), (132, 5075, 9999),
(132, 5051, 9999), (132, 5052, 9999), (132, 5053, 9999), (132, 5054, 9999), (132, 5055, 9999), (132, 5056, 9999), (132, 5057, 9999), (132, 5058, 9999), (132, 5059, 9999),
(133, 4000, 9999), (133, 4001, 9999), (133, 4002, 9999), (133, 4003, 9999), (133, 4004, 9999),
(133, 4100, 9999), (133, 4101, 9999), (133, 4102, 9999), (133, 4103, 9999), (133, 4104, 9999), (133, 4105, 9999), (133, 4106, 9999),
(133, 4200, 9999), (133, 4201, 9999), (133, 4203, 9999), (133, 4204, 9999), (133, 4205, 9999),
(133, 4300, 9999), (133, 4301, 9999), (133, 4302, 9999), (133, 4303, 9999), (133, 4304, 9999),
(133, 4400, 9999), (133, 4401, 9999), (133, 4402, 9999),
(134, 4000, 9999), (134, 4001, 9999), (134, 4002, 9999), (134, 4003, 9999), (134, 4004, 9999),
(134, 4100, 9999), (134, 4101, 9999), (134, 4102, 9999), (134, 4103, 9999), (134, 4104, 9999), (134, 4105, 9999), (134, 4106, 9999),
(134, 4200, 9999), (134, 4201, 9999), (134, 4203, 9999), (134, 4204, 9999),
(134, 4300, 9999), (134, 4301, 9999), (134, 4302, 9999), (134, 4303, 9999), (134, 4304, 9999),
(134, 4400, 9999), (134, 4401, 9999), (134, 4402, 9999);

DELETE FROM `sys_role_menu`
WHERE `menu_id` BETWEEN 4000 AND 5999
  AND `menu_id` NOT IN (
    4000, 4001, 4100, 4101, 4200, 4201, 4202, 4207,
    4400, 4405,
...
SELECT DISTINCT `role_id`, 4207, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 4202;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4405, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 4400;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4506, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 5001;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4507, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 5011;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4508, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 5012;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4509, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 5014;
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT DISTINCT `role_id`, 4510, `tenant_id` FROM `sys_role_menu` WHERE `menu_id` = 5061;

INSERT INTO `sys_role_menu` (`role_id`, `menu_id`, `tenant_id`)
SELECT 1, menu_id, 9999
FROM `sys_menu`
WHERE `menu_id` >= 4000;
SET FOREIGN_KEY_CHECKS = 1;
