...
public class AuthFilter implements GlobalFilter, Ordered
{
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain)
    {
        ...
        String tenantId = JwtUtils.getValue(claims, SecurityConstants.DETAILS_TENANT_ID);
        // The platform super administrator may select the target installer in the
        // EMS scope selector. All other users must stay on the tenant in the JWT.
        String requestedTenantId = request.getHeaders().getFirst(SecurityConstants.DETAILS_TENANT_ID);
        if ("1".equals(userid) && StringUtils.isNotEmpty(requestedTenantId))
        {
