...
public class SysRoleServiceImpl implements ISysRoleService
{
    ...
    {
        role.setTenantId(SecurityUtils.getTenantId() == null ? 9999L : SecurityUtils.getTenantId());
    ...
    @Override
    public List<SysRole> selectRolesByTenantId(Long tenantId)
    {
        return roleMapper.selectRolesByTenantId(tenantId);
    }

    @Override
    public List<SysRole> selectAssignedRolesByUserIdAndTenantId(Long userId, Long tenantId)
    {
        return roleMapper.selectAssignedRolesByUserIdAndTenantId(userId, tenantId);
    }
    ...
    public boolean checkRoleNameUnique(SysRole role)
    {
        ...
        Long tenantId = resolveRoleTenantId(role);
        SysRole info = roleMapper.checkRoleNameUnique(role.getRoleName(), tenantId, roleId);
    ...
    public boolean checkRoleKeyUnique(SysRole role)
    {
        Long roleId = StringUtils.isNull(role.getRoleId()) ? -1L : role.getRoleId();
        Long tenantId = resolveRoleTenantId(role);
        SysRole info = roleMapper.checkRoleKeyUnique(role.getRoleKey(), tenantId, roleId);
        if (StringUtils.isNotNull(info))
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    private Long resolveRoleTenantId(SysRole role)
    {
        if (StringUtils.isNotNull(role.getRoleId()))
        {
            SysRole existingRole = roleMapper.selectRoleById(role.getRoleId());
            if (StringUtils.isNotNull(existingRole) && StringUtils.isNotNull(existingRole.getTenantId()))
            {
                return existingRole.getTenantId();
            }
        }
        if (StringUtils.isNotNull(role.getTenantId()))
        {
            return role.getTenantId();
        }
        Long tenantId = SecurityUtils.getTenantId();
        return StringUtils.isNull(tenantId) ? 9999L : tenantId;
    }
    ...
    public int insertRoleMenu(SysRole role)
    {
        ...
        for (Long menuId : role.getMenuIds())
        {
            ...
            rm.setMenuId(menuId);
            rm.setTenantId(role.getTenantId() == null ? (SecurityUtils.getTenantId() == null ? 9999L : SecurityUtils.getTenantId()) : role.getTenantId());
            list.add(rm);
        }
        if (list.size() > 0)
        {
    ...
    public int insertAuthUsers(Long roleId, Long[] userIds)
    {
        ...
        for (Long userId : userIds)
        {
            ...
            ur.setUserId(userId);
            ur.setRoleId(roleId);
            ur.setTenantId(SecurityUtils.getTenantId() == null ? 9999L : SecurityUtils.getTenantId());
            list.add(ur);
        }
